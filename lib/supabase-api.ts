...
export async function getProducts(filters?: {
    ...
    if (filters?.categories && filters.categories.length > 0) {
        const orConditions = filters.categories.map(c => `category.ilike.%${c}%`).join(',')
...
export async function getCategories(filters?: { onlyMain?: boolean, parentId?: string }) {
    ...
    return data as { id: string, name: string, slug: string, name_ar?: string, parent_id?: string | null, logo_url?: string | null }[]
}
...
export async function uploadCategoryLogo(
    ...
    try {
        const fileExt = file.name.split('.').pop()
        const fileName = `category-logo-${categoryId}-${Date.now()}.${fileExt}`
        ...
        if (uploadError) {
            console.error('Error uploading category logo:', uploadError)
            return { success: false, error: uploadError.message }
        }
        ...
        console.error('Error in uploadCategoryLogo:', error)
        return { success: false, error: 'Failed to upload logo' }
    }
}

export async function updateCategoryLogo(
    categoryId: string,
    logoUrl: string | null
): Promise<{ success: boolean; error?: string }> {
    const { error } = await supabase
    ...
    if (error) {
        console.error('Error updating category logo:', error.message)
        return { success: false, error: error.message }
    }
