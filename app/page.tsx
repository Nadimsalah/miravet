        ...
        <div className="container mx-auto px-4 max-w-7xl relative z-10" dir={dir}>
          ...
          <div id="produits" className="mt-32 relative" dir={dir}>
            <div className="flex items-center justify-between mb-8">
              <h3 className="text-3xl font-black text-[#0f172a]">{t('section.best_sellers')}</h3>
              <div className="flex gap-4">
                <button
                  onClick={() => scroll('left')}
                  className="w-14 h-14 rounded-full bg-white border border-[#e2e8f0] flex items-center justify-center shadow-sm hover:scale-110 active:scale-95 transition-all text-[#0f172a] group"
                >
            ...
            <div
              ref={scrollRef}
              className="flex flex-nowrap gap-4 sm:gap-6 overflow-x-auto no-scrollbar pb-12 snap-x snap-mandatory px-4 -mx-4 sm:px-0 sm:mx-0"
            >
              {categories.length > 0 ? (
                categories.map((cat, idx) => {
                  // Dynamic icons based on category slug
                  const getIcon = (slug: string) => {
                    if (slug.includes('vaccins')) return '💉';
                    if (slug.includes('antiparasitaires')) return '🦟';
                    if (slug.includes('anti-infectieux')) return '💊';
                    if (slug.includes('anti-inflammatoires')) return '🩺';
                    if (slug.includes('vitamines')) return '🍎';
                    if (slug.includes('reproduction')) return '🧬';
                    if (slug.includes('biosecurite')) return '🧼';
                    if (slug.includes('anesthesiques')) return '💤';
                    return '📦';
                  };

                  const icon = getIcon(cat.slug);
                  const colors = ['bg-[#ffeffa]', 'bg-[#eff6ff]', 'bg-[#fffbeb]', 'bg-[#fdf2f8]', 'bg-[#ecfdf5]', 'bg-[#f0fdfa]'];

                  return (
                    <Link
                      key={cat.id}
                      href={`/category/${cat.slug}`}
                      className={`${colors[idx % colors.length]} rounded-[32px] sm:rounded-[40px] p-8 sm:p-10 min-w-[280px] sm:min-w-[400px] flex flex-col justify-between transition-all hover:-translate-y-2 lg:hover:-translate-y-4 cursor-pointer shadow-lg snap-start group`}
                    >
                      <div className="space-y-8">
                        <div className="w-[100px] h-[100px] sm:w-[140px] sm:h-[140px] rounded-full border-[6px] sm:border-[10px] border-white shadow-xl flex items-center justify-center text-4xl sm:text-6xl bg-white/40 group-hover:scale-110 transition-transform">
                          {icon}
                        </div>
                        <div className="space-y-3">
                          <h4 className="font-black text-3xl text-[#0f172a]">{isArabic && cat.name_ar ? cat.name_ar : cat.name}</h4>
                          <p className="text-base font-bold text-[#64748b] leading-relaxed">Collection {cat.name} pour professionnels</p>
                        </div>
                      </div>
                      <div className="mt-12 flex items-center justify-end">
                        <div className="w-14 h-14 rounded-full bg-white flex items-center justify-center shadow-lg group-hover:bg-[#0f172a] group-hover:text-white transition-all transform group-hover:rotate-45">
                ...
                [
                  { title: t('home.card1.title'), desc: t('home.card1.desc'), icon: '🩺', color: 'bg-[#ffeffa]' },
                  { title: t('home.card2.title'), desc: t('home.card2.desc'), icon: '✂️', color: 'bg-[#eff6ff]' },
                  { title: t('home.card3.title'), desc: t('home.card3.desc'), icon: '🦴', color: 'bg-[#fffbeb]' },
                  { title: t('home.card5.title'), desc: t('home.card5.desc'), icon: '🧪', color: 'bg-[#fdf2f8]' },
                  { title: t('home.card7.title'), desc: t('home.card7.desc'), icon: '🧼', color: 'bg-[#ecfdf5]' },
                  { title: t('home.card9.title'), desc: t('home.card9.desc'), icon: '💊', color: 'bg-[#f0fdfa]' }
                ].map((card, idx) => (
                  <div key={idx} className={`${card.color} rounded-[32px] sm:rounded-[40px] p-8 sm:p-10 min-w-[280px] sm:min-w-[400px] flex flex-col justify-between transition-all hover:-translate-y-2 lg:hover:-translate-y-4 cursor-pointer shadow-lg snap-start`}>
                    <div className="space-y-8">
                      <div className="w-[100px] h-[100px] sm:w-[140px] sm:h-[140px] rounded-full border-[6px] sm:border-[10px] border-white shadow-xl flex items-center justify-center text-4xl sm:text-6xl bg-white/40 group-hover:scale-110 transition-transform">
                        {card.icon}
                      </div>
                      <div className="space-y-3">
                        <h4 className="font-black text-3xl text-[#0f172a]">{card.title}</h4>
                        <p className="text-base font-bold text-[#64748b] leading-relaxed">{card.desc}</p>
                      </div>
                    </div>
                    <div className="mt-12 flex items-center justify-end">
                      <div className="w-14 h-14 rounded-full bg-white flex items-center justify-center shadow-lg group-hover:bg-[#0f172a] group-hover:text-white transition-all transform group-hover:rotate-45">
