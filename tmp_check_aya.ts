...
async function checkAssignmentsaya() {
  const { data: manager } = await supabase.from('profiles').select('id, name').eq('name', 'aya').single()
  if (!manager) {
    console.log('Manager aya not found')
    return
  }
  ...
  const { data: assignments } = await supabase
    ...
    .select('reseller_id, customer_id, resellers:resellers(company_name)')
